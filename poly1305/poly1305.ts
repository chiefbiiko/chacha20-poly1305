import { poly1305ClampLittleEndianBytesLittleEndianBigInt } from "./../poly1305_clamp/poly1305_clamp.ts";
import {
  littleEndianBytesToLittleEndianBigInt,
...
export function poly1305(
  ...
  const r: bigint = poly1305ClampLittleEndianBytesLittleEndianBigInt(
    littleEndianBytesToLittleEndianBigInt(otk.subarray(0, 16))
  );
  const s: bigint = littleEndianBytesToLittleEndianBigInt(otk.subarray(16, 32));
  ...
  for (let i: number = 1; i <= loopEnd; ++i) {
    ...
    b = swapBigInt(littleEndianBytesToLittleEndianBigInt(block));
